...
#     count = 0
#     while (True): 
#         coroutines = [fetchAPI(f'https://api.github.com/repos/{username}/{reponame}/commits?since={since}&until={until}&per_page={perpage}&page={page + x}', TOKEN) for x in range(DEMAND)]
...
async def fetchRepoCommitSinceUntil(username, reponame, since, until, TOKEN):
    ...
    while (True): 
        coroutines = [fetchAPI(f'https://api.github.com/repos/{username}/{reponame}/commits?since={since}&until={until}&per_page={perpage}&page={page + x}', TOKEN) for x in range(DEMAND)]
        list = await asyncio.gather(*coroutines)
        for e in list: 
            count += len(e)
        if (not all(list)): break
        page += DEMAND
    return count 
...
async def fetchRepoCommits(username, reponame, TOKEN): 
    ...
    for i in range(5): 
        ...
        until = until.strftime(FORMAT) #string format 
        since = since.strftime(FORMAT) #string format 
        commits = await fetchRepoCommitSinceUntil(username, reponame, since, until, TOKEN)
...
async def fetchRepoIssuesSince(username, reponame, since, TOKEN):
    ...
    count = 0
    while (True): 
        coroutines = [fetchAPI(f'https://api.github.com/repos/{username}/{reponame}/issues?since={since}&per_page={perpage}&page={page + x}', TOKEN) for x in range(DEMAND)]
        list = await asyncio.gather(*coroutines)
        for e in list: 
            count += len(e)
        if (not all(list)): break    
...
#fetch and caculate in one page 
async def fetchRepoPullsUnit(username, reponame, page, TOKEN): 
    RequestLine = {}
    perpage = 100
    count = 0
    current_month = datetime.now(pytz.UTC)
    previous_months = datetime.now(pytz.UTC).strftime('%B') #curent month 
    earliest_date = current_month - relativedelta(month=7)

    data = await fetchAPI(f'https://api.github.com/repos/{username}/{reponame}/pulls?per_page={perpage}&page={page}', TOKEN)
    list = np.array(data)
    if (list.size == 0): return RequestLine

    for e in list: 
        createAt = e['created_at']
        date = datetime.strptime(createAt, "%Y-%m-%dT%H:%M:%SZ")
        # check out of date
        # if (date < earliest_date): return RequestLine
...
async def fetchRepoPulls(username, reponame, TOKEN): 
    ...
    result = {}
    while (True): 
        coroutine = [fetchRepoPullsUnit(username, reponame, page+x, TOKEN) for x in range(DEMAND)]
        data = await asyncio.gather(*coroutine)
    ...
    ReponseData = CreateListOfMonth(result, 6)
    return ReponseData
