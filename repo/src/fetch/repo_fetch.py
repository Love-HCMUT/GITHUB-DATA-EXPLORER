...
async def fetch_repo_commit_since_until(owner, repo, since, until):
    """
    ...
    result = 0
    while (True): 
        data = await fetchAPI(f'https://api.github.com/repos/{owner}/{repo}/commits?since={since}&until={until}&per_page={perpage}&page={page}')
        if (len(data) == 0): break
        result += len(data)
        page += 1
    return result 

async def fetch_repo_commits(owner, repo):
    """
